...
def results(request):
    ingredients = request.session['alcohols'], request.session['otherdrinks'], request.session['fruits'], \
                  request.session['otheradds']

    selected_alcohols = Alcohol.objects.filter(id__in=ingredients[0])
    selected_otherdrinks = OtherDrink.objects.filter(id__in=ingredients[1])
    selected_fruits = Fruit.objects.filter(id__in=ingredients[2])
    selected_otheradds = OtherAdd.objects.filter(id__in=ingredients[3])

    filter_by_alcohol = Drink.objects.filter(_alcohol__in=selected_alcohols)
    filter_by_otherdrinks = Drink.objects.filter(_otherdrink__in=selected_otherdrinks)
    filter_by_fruits = Drink.objects.filter(_fruit__in=selected_fruits)
    filter_by_otheradds = Drink.objects.filter(_otheradd__in=selected_otheradds)

    checking = get_list_or_404(filter_by_alcohol | filter_by_otherdrinks | filter_by_fruits | filter_by_otheradds)
    result_list = list(chain(filter_by_alcohol, filter_by_otherdrinks, filter_by_fruits, filter_by_otheradds))
